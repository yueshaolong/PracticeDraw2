...
public class Practice14MaskFilterView extends View {
    ...
    protected void onDraw(Canvas canvas) {
        ...
        // 第二个：INNER
        paint.setMaskFilter(new BlurMaskFilter(50, Blur.INNER));
        canvas.drawBitmap(bitmap, bitmap.getWidth() + 200, 50, paint);

        // 第三个：OUTER
        paint.setMaskFilter(new BlurMaskFilter(50, Blur.OUTER));
        canvas.drawBitmap(bitmap, 100, bitmap.getHeight() + 100, paint);

        // 第四个：SOLID
        paint.setMaskFilter(new BlurMaskFilter(50, Blur.SOLID));
